...
public class MapEditorWindow : EditorWindow
{
    ...
    private List<TrashBlockSpawnData> trashBlocks = new List<TrashBlockSpawnData>();

    private enum Tool { Select, PlaceBlock, PlaceDoor, PlaceWall, PlaceTrashBlock, Eraser }
    ...
    private string blockDirection = "Up";
    ...
    private string trashBlockColor = "Red";
    private string trashBlockDirection = "Up";

    private int selectedBlockIndex = -1;
    private int selectedDoorIndex = -1;
    private int selectedWallIndex = -1;
    private int selectedTrashBlockIndex = -1;
    private bool hasSelection => selectedBlockIndex >= 0 || selectedDoorIndex >= 0 || selectedWallIndex >= 0 || selectedTrashBlockIndex >= 0;
    ...
    public List<TrashBlockSpawnData> TrashBlocks => trashBlocks;
    ...
    private void DrawToolbar()
    {
        ...
        if (GUILayout.Button("Trash", currentTool == Tool.PlaceTrashBlock ? toolButtonActiveStyle : toolButtonStyle, GUILayout.Width(80)))
            currentTool = Tool.PlaceTrashBlock;
        ...
        if (currentTool == Tool.PlaceBlock)
        {
            ...
            int blkDirIdx = System.Array.IndexOf(directions, blockDirection);
            blkDirIdx = EditorGUILayout.Popup(blkDirIdx, directions, GUILayout.Width(70));
            blockDirection = directions[blkDirIdx];
        }
        ...
        else if (currentTool == Tool.PlaceTrashBlock)
        {
            GUILayout.Label("Color:", GUILayout.Width(40));
            int colorIdx = System.Array.IndexOf(colors, trashBlockColor);
            colorIdx = EditorGUILayout.Popup(colorIdx, colors, GUILayout.Width(80));
            trashBlockColor = colors[colorIdx];

            GUILayout.Label("Dir:", GUILayout.Width(30));
            int dirIdx = System.Array.IndexOf(directions, trashBlockDirection);
            dirIdx = EditorGUILayout.Popup(dirIdx, directions, GUILayout.Width(70));
            trashBlockDirection = directions[dirIdx];
        }
        EditorGUILayout.EndHorizontal();
    }
    ...
    private void DrawInteractiveGrid()
    {
        ...
        foreach (var trashBlock in trashBlocks)
            LevelEditorGrid.DrawTrashBlock(gridOrigin, trashBlock, rows, columns);

        EditorGUILayout.Space(5);
        EditorGUILayout.LabelField($"Map: {rows}x{columns} | Blocks: {blocks.Count} | Doors: {doors.Count} | Walls: {walls.Count} | Trash: {trashBlocks.Count}", EditorStyles.centeredGreyMiniLabel);
        EditorGUILayout.EndVertical();
    }
    ...
    private void HandleGridClick(int row, int col)
    {
        switch (currentTool)
        {
            ...
            case Tool.PlaceTrashBlock: PlaceTrashBlockAt(row, col); break;
    ...
    {
        ClearSelection();
        for (int i = 0; i < trashBlocks.Count; i++)
        {
            if (trashBlocks[i].row == row && trashBlocks[i].column == col)
            {
                selectedTrashBlockIndex = i;
                return;
            }
        }
        for (int i = 0; i < blocks.Count; i++)
        {
            var footprint = LevelEditorGrid.GetBlockFootprint(blocks[i].blockType, blocks[i].direction);
    ...
    private void PlaceTrashBlockAt(int row, int col)
    {
        TrashBlockSpawnData newTrash = new TrashBlockSpawnData
        {
            row = row,
            column = col,
            color = trashBlockColor,
            direction = trashBlockDirection
        };
        if (LevelEditorGrid.HasTrashBlockOverlap(newTrash, blocks, doors, walls, trashBlocks, rows, columns))
        {
            EditorUtility.DisplayDialog("Cannot Place", "This position overlaps with an existing object or is out of bounds.", "OK");
            return;
        }
        var cmd = new PlaceObjectCommand<TrashBlockSpawnData>(trashBlocks, newTrash, trashBlocks.Count);
        commandHistory.Execute(cmd);
        ClearSelection();
        selectedTrashBlockIndex = trashBlocks.Count - 1;
    }

    private void EraseAt(int row, int col)
    {
        for (int i = trashBlocks.Count - 1; i >= 0; i--)
        {
            if (trashBlocks[i].row == row && trashBlocks[i].column == col)
            {
                var cmd = new RemoveObjectCommand<TrashBlockSpawnData>(trashBlocks, trashBlocks[i], i);
                commandHistory.Execute(cmd);
                if (selectedTrashBlockIndex == i) ClearSelection();
                else if (selectedTrashBlockIndex > i) selectedTrashBlockIndex--;
                return;
            }
        }
    ...
    private void DeleteSelected()
    {
        ...
        else if (selectedTrashBlockIndex >= 0 && selectedTrashBlockIndex < trashBlocks.Count)
        {
            var cmd = new RemoveObjectCommand<TrashBlockSpawnData>(trashBlocks, trashBlocks[selectedTrashBlockIndex], selectedTrashBlockIndex);
            commandHistory.Execute(cmd);
            ClearSelection();
        }
    }
    ...
    private void DrawGhostPreview(Rect gridOrigin)
    {
        ...
        switch (currentTool)
        {
            ...
            case Tool.PlaceWall:
                ...
                break;
            case Tool.PlaceTrashBlock:
                var ghostTrash = new TrashBlockSpawnData
                {
                    row = row,
                    column = col,
                    color = trashBlockColor,
                ...
                bool trashValid = !LevelEditorGrid.HasTrashBlockOverlap(ghostTrash, blocks, doors, walls, trashBlocks, rows, columns);
                LevelEditorGrid.DrawGhostTrashBlock(gridOrigin, ghostTrash, trashValid, rows, columns);
                break;
            case Tool.Eraser:
    ...
    private void DrawSelectionHighlight(Rect gridOrigin)
    {
        ...
        if (selectedBlockIndex >= 0 && selectedBlockIndex < blocks.Count)
        {
            var footprint = LevelEditorGrid.GetBlockFootprint(blocks[selectedBlockIndex].blockType, blocks[selectedBlockIndex].direction);
        ...
        {
            var t = trashBlocks[selectedTrashBlockIndex];
            var offsets = new List<Vector2Int> { new Vector2Int(t.column, t.row) };
            LevelEditorGrid.DrawSelectionHighlight(gridOrigin, offsets, rows, columns);
        }
    }
    ...
    private void DrawPropertyInspector()
    {
        ...
        if (selectedBlockIndex >= 0 && selectedBlockIndex < blocks.Count)
        {
            ...
            int dirIdx = System.Array.IndexOf(directions, block.direction);
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (dirIdx >= 0) ApplyProperty(block, "direction", directions[dirIdx]);
        ...
        {
            TrashBlockSpawnData trash = trashBlocks[selectedTrashBlockIndex];
            EditorGUI.BeginChangeCheck();
            int colorIdx = System.Array.IndexOf(colors, trash.color);
            colorIdx = EditorGUILayout.Popup("Color", colorIdx, colors);
            int dirIdx = System.Array.IndexOf(directions, trash.direction);
            dirIdx = EditorGUILayout.Popup("Direction", dirIdx, directions);
            int row = EditorGUILayout.IntField("Row", trash.row);
            int col = EditorGUILayout.IntField("Column", trash.column);
            if (EditorGUI.EndChangeCheck())
            {
                if (colorIdx >= 0) ApplyProperty(trash, "color", colors[colorIdx]);
                if (dirIdx >= 0) ApplyProperty(trash, "direction", directions[dirIdx]);
                if (row != trash.row) ApplyProperty(trash, "row", row);
                if (col != trash.column) ApplyProperty(trash, "column", col);
            }
            if (GUILayout.Button("Delete TrashBlock", GUILayout.Height(24))) DeleteSelected();
        }
        else
        {
    ...
    private void DrawSelectionList()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        if (GUILayout.Button("Clear TrashBlocks", GUILayout.Height(20)))
        {
            if (EditorUtility.DisplayDialog("Confirm", "Clear all trash blocks?", "Yes", "No"))
            {
                trashBlocks.Clear();
                ClearSelection();
            }
    ...
    private void SaveMap()
    {
        ...
        LevelData levelData = new LevelData
        {
            ...
            trashBlocks = new List<TrashBlockSpawnData>(trashBlocks)
        };
        LevelDataWrapper wrapper = new LevelDataWrapper { level = levelData };
        string json = JsonUtility.ToJson(wrapper, true);
        string levelsPath = System.IO.Path.Combine(Application.streamingAssetsPath, "Levels");
        if (!System.IO.Directory.Exists(levelsPath)) System.IO.Directory.CreateDirectory(levelsPath);
        string filePath = System.IO.Path.Combine(levelsPath, levelName + ".json");
    ...
    private void LoadMap()
    {
        ...
        trashBlocks = levelData.trashBlocks != null ? new List<TrashBlockSpawnData>(levelData.trashBlocks) : new List<TrashBlockSpawnData>();
        commandHistory.Clear();
        ClearSelection();
        Repaint();
