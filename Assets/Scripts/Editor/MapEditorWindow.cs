...
public class MapEditorWindow : EditorWindow
{
	...
	private void FillEmptyBorder()
	{
		...
		{
			if (!doorOccupied.Contains(new Vector2Int(c, -1)))
				topWalls.Add(new Vector2Int(c, -1));
			if (!doorOccupied.Contains(new Vector2Int(c, rows)))
				bottomWalls.Add(new Vector2Int(c, rows));
		}
		for (int r = 0; r < rows; r++)
		{
			if (!doorOccupied.Contains(new Vector2Int(-1, r)))
				leftWalls.Add(new Vector2Int(-1, r));
			if (!doorOccupied.Contains(new Vector2Int(columns, r)))
				rightWalls.Add(new Vector2Int(columns, r));
		}

		var allNewWalls = new List<WallSpawnData>();
		foreach (var p in topWalls)
			allNewWalls.Add(new WallSpawnData { row = p.y, column = p.x, direction = "Up" });
		foreach (var p in bottomWalls)
			allNewWalls.Add(new WallSpawnData { row = p.y, column = p.x, direction = "Down" });
		foreach (var p in leftWalls)
			allNewWalls.Add(new WallSpawnData { row = p.y, column = p.x, direction = "Left" });
		foreach (var p in rightWalls)
			allNewWalls.Add(new WallSpawnData { row = p.y, column = p.x, direction = "Right" });

		var existingWallSet = new HashSet<Vector2Int>();
		foreach (var w in walls)
			existingWallSet.Add(new Vector2Int(w.column, w.row));
		...
		if (added > 0)
			Debug.Log($"Fill Border: added {added} walls.");
		else
			Debug.Log("Fill Border: border already fully walled.");
