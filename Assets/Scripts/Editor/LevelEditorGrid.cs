...
public static class LevelEditorGrid
{
    ...
    private static readonly Dictionary<string, List<Vector2Int>> Footprints = new Dictionary<string, List<Vector2Int>>
    {
        ...
        { "trash_block", new List<Vector2Int> { new Vector2Int(0, 0) } }
    };
    ...
    {
        return GetBlockFootprint(blockType, "Up");
    }

    public static List<Vector2Int> GetBlockFootprint(string blockType, string direction)
    {
        if (!Footprints.TryGetValue(blockType, out var baseFootprint))
            return new List<Vector2Int> { new Vector2Int(0, 0) };

        if (string.IsNullOrEmpty(direction) || direction == "Up")
            return new List<Vector2Int>(baseFootprint);

        return RotateFootprint(baseFootprint, direction);
    }
    ...
    private static List<Vector2Int> RotateFootprint(List<Vector2Int> footprint, string direction)
    {
        ...
        for (int i = 0; i < footprint.Count; i++)
        {
            ...
            switch (direction)
            {
                case "Right": nx = rel.y; ny = -rel.x; break;
                case "Down": nx = -rel.x; ny = -rel.y; break;
                case "Left": nx = -rel.y; ny = rel.x; break;
                default: nx = rel.x; ny = rel.y; break;
            }
    ...
    public static Vector2Int GetBlockDimensions(string blockType)
    {
        return blockType switch
        {
            ...
            "trash_block" => new Vector2Int(1, 1),
    ...
    {
        var footprint = GetBlockFootprint(block.blockType, block.direction);
        foreach (var offset in footprint)
        {
            int r = block.row + offset.y;
            int c = block.column + offset.x;
            if (!IsInBounds(r, c, rows, columns)) return true;
            foreach (var other in blocks)
            {
                var otherFootprint = GetBlockFootprint(other.blockType, other.direction);
                foreach (var otherOffset in otherFootprint)
                {
                    if (other.row + otherOffset.y == r && other.column + otherOffset.x == c) return true;
                }
            }
            foreach (var door in doors)
            {
                if (door.row == r && door.column == c) return true;
            }
            foreach (var wall in walls)
            {
                if (wall.row == r && wall.column == c) return true;
            }
        }
        return false;
    }
    ...
    public static bool HasDoorOverlap(DoorSpawnData door, List<BlockData> blocks, List<DoorSpawnData> doors, List<WallSpawnData> walls, int rows, int columns)
    {
        ...
        foreach (var offset in doorFootprint)
        {
            ...
            if (!IsDoorInBounds(r, c, rows, columns)) return true;
            foreach (var block in blocks)
            {
                var footprint = GetBlockFootprint(block.blockType, block.direction);
                foreach (var blockOffset in footprint)
                {
                    if (block.row + blockOffset.y == r && block.column + blockOffset.x == c) return true;
                }
            }
    ...
    {
        if (!IsInBounds(wall.row, wall.column, rows, columns) && !IsDoorBorderCell(wall.row, wall.column, rows, columns)) return true;
        foreach (var block in blocks)
        {
            var footprint = GetBlockFootprint(block.blockType, block.direction);
            foreach (var offset in footprint)
            {
                if (block.row + offset.y == wall.row && block.column + offset.x == wall.column) return true;
            }
        }
        foreach (var door in doors)
        {
            ...
            foreach (var offset in footprint)
            {
                if (door.row + offset.y == wall.row && door.column + offset.x == wall.column) return true;
            }
        }
        foreach (var other in walls)
        {
            if (wall.row == other.row && wall.column == other.column) return true;
        }
        return false;
    }

    public static bool HasTrashBlockOverlap(TrashBlockSpawnData trashBlock, List<BlockData> blocks, List<DoorSpawnData> doors, List<WallSpawnData> walls, List<TrashBlockSpawnData> trashBlocks, int rows, int columns)
    {
        int r = trashBlock.row;
        int c = trashBlock.column;
        if (!IsInBounds(r, c, rows, columns)) return true;
        foreach (var block in blocks)
        {
            var footprint = GetBlockFootprint(block.blockType, block.direction);
            foreach (var offset in footprint)
            {
                if (block.row + offset.y == r && block.column + offset.x == c) return true;
            }
        }
        foreach (var door in doors)
        {
            ...
            foreach (var offset in footprint)
            {
                if (door.row + offset.y == r && door.column + offset.x == c) return true;
            }
        }
        foreach (var wall in walls)
        {
        ...
        foreach (var other in trashBlocks)
        {
            if (other == trashBlock) continue;
            if (other.row == r && other.column == c) return true;
        }
        return false;
    }
    ...
    public static List<string> GetAllValidationErrors(LevelData level)
    {
        ...
        for (int i = 0; i < level.walls.Count; i++)
        {
            ...
            if (!IsInBounds(wall.row, wall.column, level.rows, level.columns) && !IsDoorBorderCell(wall.row, wall.column, level.rows, level.columns))
            {
        ...
        if (level.trashBlocks != null)
        {
            for (int i = 0; i < level.trashBlocks.Count; i++)
            {
                var trash = level.trashBlocks[i];
                if (!IsInBounds(trash.row, trash.column, level.rows, level.columns))
                {
                    errors.Add($"TrashBlock at ({trash.row},{trash.column}) out of bounds");
                }
                for (int j = i + 1; j < level.trashBlocks.Count; j++)
                {
                    var other = level.trashBlocks[j];
                    if (trash.row == other.row && trash.column == other.column)
                    {
                        errors.Add($"TrashBlock at ({trash.row},{trash.column}) overlaps with another TrashBlock");
                        break;
                    }
                }
                foreach (var block in level.blocks)
                {
                    ...
                    foreach (var offset in footprint)
                    {
                        if (block.row + offset.y == trash.row && block.column + offset.x == trash.column)
                        {
                            errors.Add($"TrashBlock at ({trash.row},{trash.column}) overlaps with Block {block.blockType}");
                            break;
                        }
                    }
                }
                foreach (var door in level.doors)
                {
                    ...
                    foreach (var offset in footprint)
                    {
                        if (door.row + offset.y == trash.row && door.column + offset.x == trash.column)
                        {
                            errors.Add($"TrashBlock at ({trash.row},{trash.column}) overlaps with Door {door.doorType}");
                            break;
                        }
                    }
                }
                foreach (var wall in level.walls)
                {
                    if (wall.row == trash.row && wall.column == trash.column)
                    {
                        errors.Add($"TrashBlock at ({trash.row},{trash.column}) overlaps with Wall");
                        break;
                    }
                }
            }
    ...
    {
        var footprintA = GetBlockFootprint(a.blockType, a.direction);
        var footprintB = GetBlockFootprint(b.blockType, b.direction);
        foreach (var offsetA in footprintA)
        {
            ...
            {
                if (b.row + offsetB.y == rA && b.column + offsetB.x == cA) return true;
            }
        }
        return false;
    }
    ...
    private static bool BlockOverlapsDoor(BlockData block, DoorSpawnData door)
    {
        ...
        foreach (var offset in footprint)
        {
            if (block.row + offset.y == door.row && block.column + offset.x == door.column) return true;
        }
        return false;
    }
    ...
    public static void DrawWall(Rect gridOrigin, WallSpawnData wall, int rows, int columns)
    {
        ...
        if (!IsInBounds(mapRow, mapColumn, rows, columns) && !IsDoorBorderCell(mapRow, mapColumn, rows, columns)) return;
        int visualRow = mapRow + 1;
        int visualCol = mapColumn + 1;
    ...
    public static void DrawTrashBlock(Rect gridOrigin, TrashBlockSpawnData trashBlock, int rows, int columns)
    {
        int r = trashBlock.row;
        int c = trashBlock.column;
        if (!IsInBounds(r, c, rows, columns)) return;
        int visualRow = r + 1;
        int visualCol = c + 1;
        Rect cellRect = new Rect(
            gridOrigin.x + visualCol * CellSize,
            gridOrigin.y + visualRow * CellSize,
            CellSize, CellSize);
        EditorGUI.DrawRect(cellRect, new Color(0.55f, 0.35f, 0.1f, 0.8f));
        DrawCellBorder(cellRect, new Color(0.8f, 0.6f, 0.2f));
        GUI.Label(cellRect, "T", EditorStyles.centeredGreyMiniLabel);
    }
    ...
    {
        var footprint = GetBlockFootprint(ghostBlock.blockType, ghostBlock.direction);
        bool allValid = isValid;
        foreach (var offset in footprint)
        {
            int r = ghostBlock.row + offset.y;
            int c = ghostBlock.column + offset.x;
            if (!IsInBounds(r, c, rows, columns)) allValid = false;
        }
    ...
    public static void DrawGhostWall(Rect gridOrigin, WallSpawnData ghostWall, bool isValid, int rows, int columns)
    {
        ...
        bool inBounds = IsInBounds(r, c, rows, columns) || IsDoorBorderCell(r, c, rows, columns);
        if (!inBounds) return;
        int visualRow = r + 1;
        int visualCol = c + 1;
        Rect cellRect = new Rect(
    ...
    public static void DrawGhostTrashBlock(Rect gridOrigin, TrashBlockSpawnData ghostTrash, bool isValid, int rows, int columns)
    {
        int r = ghostTrash.row;
        int c = ghostTrash.column;
        ...
        Rect cellRect = new Rect(
            ...
            gridOrigin.y + visualRow * CellSize,
            CellSize, CellSize);
        Color fill = isValid ? new Color(0.55f, 0.35f, 0.1f, 0.35f) : new Color(1f, 0.2f, 0.2f, 0.35f);
        Color border = isValid ? new Color(0.8f, 0.6f, 0.2f) : Color.red;
        EditorGUI.DrawRect(cellRect, fill);
        DrawCellBorder(cellRect, border);
    }
