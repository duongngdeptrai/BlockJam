...
{
if (!IsInBounds(wall.row, wall.column, rows, columns) && !IsDoorBorderCell(wall.row, wall.column, rows, columns))
return true;
foreach (var block in blocks)
{
var footprint = GetBlockFootprint(block.blockType, block.direction);
...
foreach (var offset in footprint)
{
if (door.row + offset.y == wall.row && door.column + offset.x == wall.column)
return true;
}
}
